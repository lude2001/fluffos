...
"%BASH_EXE%" -lc "set -euo pipefail; export MSYSTEM=MINGW64; export PATH=/mingw64/bin:/usr/bin:$PATH; root=$(cygpath -m \"$1\"); build=$(cygpath -m \"$2\"); msys=$(cygpath -m \"$3\"); jobs=\"$4\"; cmake -S \"$root\" -B \"$build\" -G 'MSYS Makefiles' -DCMAKE_BUILD_TYPE=RelWithDebInfo -DMARCH_NATIVE=OFF -DENABLE_LUDE_COMPILE_SERVICE=ON -DPACKAGE_CRYPTO=OFF -DPACKAGE_DB=ON -DPACKAGE_DB_MYSQL=1 -DPACKAGE_DB_SQLITE=2 -DPACKAGE_DB_DEFAULT_DB=1 -DMYSQL_INCLUDE_DIR=\"$msys/mingw64/include/mysql\" -DMYSQL_LIB_DIR=\"$msys/mingw64/lib\" \"-DMYSQL_EXTRA_LIBRARIES=$msys/mingw64/lib/libcurl.dll.a;$msys/mingw64/lib/libzstd.dll.a;secur32;shlwapi\"; cmake --build \"$build\" --target driver lpcprj lpccp -j \"$jobs\"" -- "%ROOT_DIR%" "%BUILD_DIR%" "%MSYS2_ROOT%" "%JOBS%"
