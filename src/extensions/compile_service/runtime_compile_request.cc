...
std::vector<CompileServiceDiagnostic> collect_compile_diagnostics() {
  std::vector<CompileServiceDiagnostic> result;
  result.reserve(compiler_diags.size());
  for (const auto &diagnostic : compiler_diags) {
    result.push_back(CompileServiceDiagnostic{
        diagnostic.is_warning ? "warning" : "error",
        diagnostic.file.empty() ? std::string{} : add_slash(diagnostic.file.c_str()),
        diagnostic.line,
        diagnostic.column,
...
CompileServiceResponse compile_single_file(const std::string &target, std::string_view mode) {
  ...
  try {
    ...
    auto message = current_error_message();
  ...
  response.diagnostics = collect_compile_diagnostics();
