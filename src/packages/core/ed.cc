...
static void count_blanks(int line) { _count_blanks(gettxtl(getptr(line)), 0); }
...
static int doprnt(int from, int to) {
  ...
  if (to != 0) {
    setCurLn(from);
    while (P_CURLN <= to) {
      count = prntln(gettxtl(P_CURPTR), tmp, P_LFLG, (P_NFLG ? P_CURLN : 0));
      if (P_NET_DEAD) {
        break;
      }
#if (BUFFER_SIZE) > (ED_MAXLINE)
...
static void shift(char *text) {
  ...
  while (*text == ' ' || *text == '\t') {
    if (*text++ == ' ') {
      ...
      indent_index = indent_index + ED_TAB_WIDTH - (indent_index % ED_TAB_WIDTH);
    }
  }
  ...
  while (*p != '\0') {
            ...
            if (do_indent) {
              ...
              for (q = start; q < p - 1;) {
                if (*q++ == '\t') {
                  ...
                  // index2 = (index2 + 8) & ~7;
                  indent_index = indent_index + ED_TAB_WIDTH - (indent_index % ED_TAB_WIDTH);
                  index2 = index2 + ED_TAB_WIDTH - (index2 % ED_TAB_WIDTH);
    ...
    for (;;) {
      ...
      if (f[top] <= g[token]) { /* shift the token on the stack */
        ...
        if ((token == LBRACKET && (*sp == ROPERATOR || *sp == ELSE || *sp == XDO)) ||
        ...
        if (do_indent) {
          ...
          if (token == TOKEN && *sp == LBRACKET &&
              (strcmp(ident, "case") == 0 || strcmp(ident, "default") == 0)) {
    ...
    case 'z':
      ...
      switch (*inptr) {
        case '-':
          ...
          if ((st = doprnt(P_LINE1 - (scr), P_LINE1 + ED_BUFFER->scroll_lines - (scr + 2))) < 0) {
            return st;
          }
          break;
 ...
 */
#ifdef OLD_ED
void ed_start(const char *file_arg, const char *write_fn, const char *exit_fn, int restricted,
              object_t *exit_ob, int scroll_lines) {
  svalue_t *setup;
      ...
      if (outstr) {  // We need to figure out the paging
        ...
        while (*brkpt != '\0') {
          ...
          if ((*brkpt == '\n') || (*brkpt == '\0')) {
            ...
            if ((lines == (ED_BUFFER->scroll_lines - 1)) || (*brkpt == '\0')) {
              if (*brkpt == '\0') {
                tmpc = '\0';
              } else {
...
char *object_ed_start(object_t *ob, const char *fname, int restricted, int scroll_lines) {
  svalue_t *setup;
